...
namespace Xe.Game.Particles
{
	public class Effect : IEffect
	{
		...
		{
			return Get(Ease, x * Speed + FixStep) * Multiplier + Sum;
		}

		public static double Get(Ease ease, double x)
		{
			double f;
			switch (ease)
			{
				...
				case Ease.BounceEaseIn: return 1 - Get(Ease.BounceEaseOut, 1 - x);
				...
				case Ease.BounceEaseInOut:
					if (x < 0.5)
						return 0.5 * Get(Ease.BounceEaseIn, x * 2);
					else
						return 0.5 * Get(Ease.BounceEaseOut, x * 2 - 1) + 0.5;
				default: return 0.0;
			}
		}
	}
